...
from Dynamic.sysODE import cip, pend_cart_nl
...
from cip_acados import create_acados_ocp_cartpole2
...
def mpc_ctrl2(x, ref, p, u_prev):
    """
    trying to implement control rate restriction
    ctrl_rate <= (u - u_prev) / dt
    """
    global ocp_solver
    ...
    if ocp_solver is None:
        build_dir = "cip_build2"
        dll_path = os.path.join(build_dir, "acados_ocp_solver_cartpole_mpc.dll")
        json_path = 'acados_ocp2.json'

        ocp = create_acados_ocp_cartpole2(dt, N, p)
    ...
    for i in range(N):
        ocp_solver.set(i, "p", u_prev)  # Set u_prev for ∆u constraint
...
def main():
    ...
    for i in range(Nt):
        ...
        # Define control
        start = time()
        u_ = mpc_ctrl2(xk, ref, p, u_prev)       # MPC
    ...
    opt_3 = {0: {"xlabel": "Time, s", "ylabel": "F [N]", "label": "Force"},
             1: {"xlabel": "Time, s", "ylabel": "dF/dt [N/s]", "label": "Force Rate"}}
    ...
    show_results(tspan, np.vstack((u_hist, du_hist)), options=opt_3, title="Control")
    plt.show(block=True)
