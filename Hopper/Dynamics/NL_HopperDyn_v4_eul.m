function [xdot, DCM, G] = NL_HopperDyn_v4_eul(x, u, Jo, Jdot, mdot, p, d)
...
%% Parameters
% Jxx = p(1); Jyy = p(2); Jzz = p(3); Jxy = p(4); Jyz = p(5); Jxz = p(6); 
% cx = p(7); cy = p(8); cz = p(9); cr = p(10); cq = p(11); cp = p(12); 
% gg = p(13); beta = p(14); gamma   = p(15); rho = p(16);
% k_quat  = p(20);      % for quaternion normalization
% gc = p(21);           % turnON garvity
%% Mass and Inertia matrices
Mass = diag([x(13), x(13), x(13)]);
%% Damping matrices
Cv = diag([p(7), p(7), p(7)]);
Cw = diag([p(10), p(10), p(10)]);
% Cv = diag([0, 0, 0]);
% Cw = diag([0, 0, 0]);
%% DCM
DCM = Nw(a);                              % inertial to body DCM
% L2B = rotx(a(1))*roty(a(3))*rotz(a(2)); % L2B - DCM = zeros(3)
%% Gravity
G = p(21) * DCM * [0; -p(13); 0];                          % Get gravity in body frame    
%% System of Equations 
